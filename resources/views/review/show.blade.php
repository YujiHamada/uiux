...
@section('content')
  ...
  @if(Auth::user()->id != $review->user_id)
    ...
    <p>↓このレビューに↓</p>
    <button id="agree" class="agree btn btn-primary {{isset($agree)?' clicked':''}}" type="button" value="{{Config::get('enum.agree.AGREE')}}">賛成@if(isset($agree) && $agree->is_agree == 1)済@endif</button>
    <button id="disagree" class="agree btn btn-warning {{isset($agree)?' clicked':''}}" type="button" value="{{Config::get('enum.agree.DISAGREE')}}">反対@if(isset($agree) && $agree->is_agree == 0)済@endif</button>
  @endif
@endsection
