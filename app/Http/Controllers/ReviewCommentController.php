...
class ReviewCommentController extends Controller
{
    ...
    public function evaluate(Request $request) {
        $commentEvaluation = Comment_Evaluation::where('comment_id', $request->comment_id)->where('user_id', $request->user_id)->first();

        if(!empty($commentEvaluation)){
            //すでにレビューに対する評価があったらその評価を削除して削除フラグを返却する
            Comment_Evaluation::where('comment_id', $request->comment_id)->where('user_id', $request->user_id)->delete();
            ...
            $commentEvaluation = new Comment_Evaluation;
